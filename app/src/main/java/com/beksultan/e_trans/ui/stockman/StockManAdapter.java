...
public class StockManAdapter extends BaseRViewAdapter {

    private static final int LAYOUT_ID = R.layout.item_order_stockman;

    private Callback callback;

    public StockManAdapter() {
    }
    ...
    protected int getLayoutResourceId() {
        return R.layout.item_order_stockman;
    }
    ...
    public MainViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch (viewType) {
            case LAYOUT_ID:
                return new StockManHolder(inflate(parent, LAYOUT_ID));
    ...
    public void onBindViewHolder(@NonNull MainViewHolder holder, int position) {
        switch (holder.getItemViewType()) {
            case LAYOUT_ID:
                ((StockManHolder) holder).bind((Waybill) data.get(position));
                break;
        }
    }
    ...
    public class StockManHolder extends MainViewHolder {

        @BindView(R.id.ll_send_waybill)
        LinearLayout ll_send_waybill;
        ...
        private Waybill bindWaybill;

        public StockManHolder(View itemView) {
            super(itemView);
        ...
        @OnClick(R.id.ll_send_waybill)
        public void onClickSend() {
            callback.send(bindWaybill);
        }
    }

    public interface Callback {
        void send(Waybill waybill);
    }
}
